...
const FGTSReceiptGenerator = () => {
  ...
  const previewExtractedInfo = () => {
    ...
    try {
      ...
      const convenioMatch = textContent.match(/Codigo do Convenio\s*:\s*(\d+)/i);
      const competenciaMatch = textContent.match(/Data de competencia\s*:\s*(\d{2})(\d{4})/i);
      ...
      if (valorDocumentoMatch || codigoBarrasMatch || nsuMatch) {
        ...
        if (competenciaMatch && competenciaMatch[1] && competenciaMatch[2]) {
  ...
  const generatePDF = async () => {
    ...
    const modifiedHtmlContent = htmlContent
      ...
      .replace(
        '<td id="convenio"></td>',
        `<td class="foco" id="convenio">${convenioformatado}</td>`
      )
      .replace(
        '<td id="nsu"></td>',
    ...
    return html2pdf(tempElement, {
      margin: [10, 10, 10, 10],
      filename: "comprovante_fgts_" + nsuMatch[1] + ".pdf",
              ...
              {!isDataExtracted ? (
                ...
                <div className={styles.extractedInfo}>
                  <div className={styles.infoGrid}>
                    ...
                    <div className={styles.infoItem}>
                      <span className={styles.infoLabel}>Competência:</span>
                      <span className={styles.infoValue}>
                        {extractedData.competencia}
                      </span>
                    </div>
